...
def rotate_plate_image(contour, plate_image):
    ...
    enlarged_width = int(w * 3 / 2)
    enlarged_height = int(h * 3 / 2)
    enlarged_image = np.zeros((enlarged_height, enlarged_width, plate_image.shape[2]), dtype=plate_image.dtype)
    x_in_enlarged = int((enlarged_width - w) / 2)
    y_in_enlarged = int((enlarged_height - h) / 2)
    ...
    # 获取执行旋转所需的变换矩阵
    transform_matrix = cv.getRotationMatrix2D(new_center, angle + 180, 1.0)   # 角度为正，表明逆时针旋转
    # 执行旋转
